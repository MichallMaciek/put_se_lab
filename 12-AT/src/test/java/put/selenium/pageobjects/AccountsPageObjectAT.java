...
public class AccountsPageObjectAT {
    ...
    public void successfulUserRegistration() throws Exception {
        ...
        assertTrue("We should be on the home page", mainPage.isOnPageNotLoggedIn());

        mainMenu.clickRegisterLink();

        assertTrue("We should be on the registration page", registrationPage.isOnPage());
        registrationPage.registerUser("user", "password", "password", "John Doe", "10 New St.");
        assertTrue("We should be redirected to the login page", loginPage.isOnPage());
        loginPage.loginUser("user", "password");

        String expectedUserInfo = "John Doe, 10 New St.";
        assertEquals(expectedUserInfo, loggedInUserInfo.getUserInformation());
    }
